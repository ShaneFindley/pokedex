...
export const PokemonActions: React.FC<{pokemon: IEnrichedDetail}> = ({pokemon}) => {
    ...
    const myPokemonAction = inMyPokemon
        ? <Button className='action' onClick={(e: React.MouseEvent<HTMLButtonElement>) => { e.stopPropagation(); dispatch(remove(pokemon)); }}>Remove from My Pokemon</Button>
        : <Button className='action' onClick={(e: React.MouseEvent<HTMLButtonElement>) => { e.stopPropagation(); dispatch(add(pokemon)); }}>Add to My Pokemon</Button>
